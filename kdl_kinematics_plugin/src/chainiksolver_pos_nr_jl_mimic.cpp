  ...
  , position_ik(_position_ik)
{
  ...
  CONSOLE_BRIDGE_logDebug("kdl Limits");
  for (std::size_t i = 0; i < q_min.rows(); ++i)
  {
    CONSOLE_BRIDGE_logDebug("kdl %ld: Min: %f, Max: %f", long(i), q_min(i), q_max(i));
  }
  CONSOLE_BRIDGE_logDebug("kdl  ");
}
...
bool ChainIkSolverPos_NR_JL_Mimic::setMimicJoints(const std::vector<kdl_kinematics_plugin::JointMimic>& _mimic_joints)
{
  ...
  {
    CONSOLE_BRIDGE_logError("kdl Mimic Joint info should be same size as number of joints in chain: %d",
                    chain.getNrOfJoints());
    return false;
  }
  ...
  for (std::size_t i = 0; i < _mimic_joints.size(); ++i)
  {
    ...
    {
      CONSOLE_BRIDGE_logError("kdl Mimic Joint index should be less than number of joints in chain: %d",
                      chain.getNrOfJoints());
      return false;
    }
  }
  ...
  CONSOLE_BRIDGE_logDebug("kdl Set mimic joints");
  return true;
}
                                                    ...
                                                    bool lock_redundant_joints)
{
  ...
  q_temp = q_init;
  CONSOLE_BRIDGE_logDebug("kdl Input:");
  for (std::size_t i = 0; i < q_out.rows(); ++i)
    CONSOLE_BRIDGE_logDebug("kdl %d: %f", (int)i, q_out(i));
  ...
  for (i = 0; i < maxiter; ++i)
  {
    ...
    CONSOLE_BRIDGE_logDebug("kdl delta_twist");
    for (std::size_t i = 0; i < 6; ++i)
      CONSOLE_BRIDGE_logDebug("kdl", "%d: %f", (int)i, delta_twist(i));

    iksolver.CartToJnt(q_temp, delta_twist, delta_q);

    Add(q_temp, delta_q, q_temp);

    CONSOLE_BRIDGE_logDebug("kdl delta_q");
    for (std::size_t i = 0; i < delta_q.rows(); ++i)
      CONSOLE_BRIDGE_logDebug("kdl", "%d: %f", (int)i, delta_q(i));

    CONSOLE_BRIDGE_logDebug("kdl q_temp");
    for (std::size_t i = 0; i < q_temp.rows(); ++i)
      CONSOLE_BRIDGE_logDebug("kdl", "%d: %f", (int)i, q_temp(i));
  ...
  //  qMimicToq(q_temp, q_out);
  q_out = q_temp;
  CONSOLE_BRIDGE_logDebug("kdl Full Solution:");
  for (std::size_t i = 0; i < q_temp.rows(); ++i)
    CONSOLE_BRIDGE_logDebug("kdl", "%d: %f", (int)i, q_temp(i));

  CONSOLE_BRIDGE_logDebug("kdl Actual Solution:");
  for (std::size_t i = 0; i < q_out.rows(); ++i)
    CONSOLE_BRIDGE_logDebug("kdl", "%d: %f", (int)i, q_out(i));
